...
#define TIMETOSENDPGN 0.80              //in seconds
#define MBD "/dev/ttyUSB0"
...
static int ReadAnemometer(int sog, RawMessage *msg, modbus_t *socket);
static int  OpenInitMB(char *device, modbus_t *mbsocket);
...
while(1)
{
   ctx=modbus_new_rtu(MBD, 9600, 'N', 8, 1);
   	if (ctx == NULL) {
               fprintf(stderr, "Unable to create the libmodbus context\n");
               exit(1);
           }

  if(OpenInitMB(MBD, ctx)==-1) exit(1);
  else	printf("MBD - Open\n");
 ...
 while (++resetSoft)
 {
        ...
        if((tscur-tsold)>TIMETOSENDPGN) //delay is > TIMETOSENDPGN - time send new wind parameter
        {
         ...
         if(sog>100)		//sog > 1m/s (calculate and send tru wind)
	  if(ReadAnemometer(sog, &m, ctx)>0)       //read data from anemometr
           writeRawPGNToCanSocket(&m, socket);    //send true wind to can
	 
         if(ReadAnemometer(0, &m, ctx)>0)          //read data from anemometr
          writeRawPGNToCanSocket(&m, socket);    //send appear wind to can
        };
...
static int ReadAnemometer(int sog, RawMessage *msg, modbus_t *mbsocket)   //if sog>100 (1m/s) send wind reference on boat else send wind true
{
...
//   msg->data[4]=0;            //direct H
   msg->data[5]=2;              //2 - reference dir wind, 3 - true wind
   msg->data[6]=0;
   msg->data[7]=0;

   //read wind and direction from rs (device) 
   while(rc<1 && i--)
    rc = modbus_read_registers(mbsocket, 0, 3, tab_reg);


   if (rc == -1) {
            fprintf(stderr, "%s\n", "ERROR2");
            return -1; //not read wind - return
}
    wind_speed=tab_reg[0];
    wind_dir=(tab_reg[1]*31415.9265358979323846264338327950288/180.0);
...
static int  OpenInitMB(char *device, modbus_t *socket)
{
 int rc=0; 
 int modaddr=2; //default addr anemometr
 uint16_t tab_reg[4];
 unsigned char i=5;

modbus_set_slave(socket,0);	//set addr divece as 0
	if (modbus_connect(socket) == -1) 
		{
		 fprintf(stderr, "Connection failed: %s\n", "ERROR"); 
                 modbus_free(socket);
                 return -1;
		}
	while(rc<1 && i--) 	//while i>0 or not read modbus
	 rc = modbus_read_registers(socket, 0x1000, 1, tab_reg); //read addr device
	if (rc == -1)
		{
		 fprintf(stderr, "%s\n", "Read Error address");
		 return -1;
	...
	modaddr=tab_reg[0];     //read addr device
         modbus_set_slave(socket,modaddr);     //set to addr device 
 return modaddr;
}
